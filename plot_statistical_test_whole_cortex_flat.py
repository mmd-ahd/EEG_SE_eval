"""
Composite Visualization of Pairwise Statistical Clusters (Whole-Brain Flatmap).
...
1. Renders whole-brain (both hemispheres) side-by-side using a 'split' view.
2. Uses the 2D 'flat' surface projection instead of 3D 'inflated'.
3. 'pos_lims' is used to ensure low-t-statistic clusters do not fade into 
   the white background, forcing them to render with high-contrast colors.
4. Figure dimensions and grid spacing (hspace) have been expanded to accommodate
   the wider flatmap images and entirely prevent text overlapping.
"""
...
for m1, m2 in method_pairs:
    ...
    for metric in metrics:
        fname = os.path.join(stats_dir, f"Stats_Pairwise_{m1}_vs_{m2}_{metric}")
    ...
    out_fname = os.path.join(screenshots_dir, f'Whole_Brain_Clusters_Flatmap_{m1}_vs_{m2}_{SPACING}.png')
