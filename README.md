...
The analysis is configured to run on the openly available **ds003505** dataset (formatted using the Brain Imaging Data Structure, BIDS). It utilizes preprocessed EEGLAB datasets (`.set`) for 19 subjects performing Visual Evoked Potential tasks (Faces vs. Motion).

To use this repository, please download the dataset and place it in the appropriate directory (by default: `ds003505/derivatives`).

## Dependencies

The scripts are written in Python 3 and heavily rely on the MNE-Python ecosystem. To run the pipeline, you will need the following core packages:
...
```bash
pip install mne numpy pandas matplotlib seaborn pyvista pyvistaqt
```
...
To replicate the analyses presented in the study, follow these steps:

1. **Data Preparation**:
   Ensure your dataset is located in the directory specified by the `dataset_dir` and `derivatives_dir` variables within the scripts (default: `ds003505/derivatives`).

2. **Modeling Pipeline**:
   Run the preprocessing and modeling scripts sequentially:
   ```bash
...
3. **Compute Metrics**:
   Calculate the resolution metrics for each subject and then compute the grand average:
   ```bash
   python resolution_metrics.py
   python grand_average_metrics.py
   ```

4. **Analyze ROIs**:
   Evaluate the metrics specifically for the functional regions of interest:
   ```bash
   python ROI_based_metrics.py
   python ROIs.py
   ```

5. **Generate Visualizations**:
   Create the final plots and figures for analysis:
   ```bash
   python plot_metrics_table.py
   python plot_hist.py
   ```
